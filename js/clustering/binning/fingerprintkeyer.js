var FingerprintKeyer = function() {};

FingerprintKeyer.prototype.key = function(s) {
	if (typeof s !== 'string') {
		throw new IllegalArgumentException('Fingerprint keyer accepts a single string parameter');
	}
	s = s.toLowerCase();
	s = s.replace(/[!"#$%&'*+,\-.\/\\:;<=>?@\[\]\^_`{|}~]|[\x00-\x08\x0A-\x1F\x7F]/g, '');
	s = s.replace(/\s+/g, ' ');
	s = s.trim();
...
FingerprintKeyer.prototype.asciify = function(s) {
	var c = s.split();
	var b = '';
	for (var i in c) {
 ...
 */
FingerprintKeyer.prototype.translate = function(c) {
	switch(c) {
		case '\u00C0':
		case '\u00C1':
		case '\u00C2':
